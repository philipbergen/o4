...
from o4_pyforce import (
    Pyforce,
    P4Error,
    P4TimeoutError,
    info as pyforce_info,
    client as pyforce_client,
    clear_cache,
)
from o4_fstat import (
    fstat_from_csv,
    fstat_iter,
    fstat_path,
    fstat_split,
...
def o4_seed_from(seed_dir, seed_fstat, op):
    ...
    def no_uchg(*fnames):
        check_call(
            ["chflags", "nouchg"] + [fname for fname in fnames if os.path.exists(fname)]
        )
    ...
    if seed_fstat:
        seed_checksum = {
            ...
            for f in fstat_from_csv(seed_fstat, fstat_split)
            if f
        }
...
def o4_drop_have(verbose=False):
    ...
    for line in lines:
        ...
        if not have:
            ...
            have = check_output(
                ["p4", "have", "..."], encoding=sys.stdout.encoding, stderr=DEVNULL
            )
            if verbose:
                t0, t1 = time.time(), t0
...
def o4_filter(filtertype, filters, verbose):
    ...
    def f_existence(row):
        ...
        """
        return (
            os.path.lexists(row[F_PATH]) and not os.path.isdir(row[F_PATH])
        ) == bool(row[F_CHECKSUM])
    ...
    def f_checksum(row):
        if os.path.lexists(row[F_PATH]):
            if not row[F_CHECKSUM]:  # File is deleted
                ...
                row[F_FILE_SIZE].endswith("symlink")
                or Pyforce.checksum(row[F_PATH], row[F_FILE_SIZE]) == row[F_CHECKSUM]
            ):
                return True
        else:
    ...
    funcs = [
        inverter(f"f_{fname}(x)", invert) for fname, doit, invert in filters if doit
    ]
    if not funcs:
...
def o4_sync(
    ...
    if os.path.exists(SYNCED_CL_FILE):
        with open(SYNCED_CL_FILE) as fin:
            try:
                ...
                err_print(
                    f"{CLR}*** WARNING: {os.getcwd()}/{SYNCED_CL_FILE} could not be read"
                )
    ...
    progress = (
        f"| {o4bin} progress" if sys.stdin.isatty() and progress_enabled() else ""
    )
    pyforce = (
        "pyforce"
    )  # pyforce = 'pyforce' + (' --debug --' if os.environ.get('DEBUG', '') else '')
    ...
    if is_git_hybrid():
        ...
        pyforce = (
            f"{pyforce} --writable --"
        )  # Make sure this is not affixed for opened files
    ...
    cmd = (
        f"{fstat} | {o4bin} drop --not-deletes --existence"
        f"{keep_case}"
        f"{progress}"
        f"{syncit}"
        f"{retry}"
    ...
    if seed:
        if not previous_cl:
            ...
            print(
                "*** INFO: Flushing took {:.2f} minutes".format((time.time() - t0) / 60)
            )
    ...
    if prep:
        err_print(
            "*** INFO: Sync from p4 is complete. Hybrid import upstream changes to git."
        )
        git_o4_import(prep)
...
def o4_clean(changelist, quick=False, resume=False, discard=False):
    ...
    check_call(cmd)
    if not discard:
        savedir = source.replace(
            "cleaning", time.strftime("cleaned")
        )  # @%Y-%m-%d,%H:%M'))
        shutil.move(source, savedir)
        err_print(
            f"*** INFO: Directory is clean @{changelist}; detritus is in {savedir}"
        )
    else:
...
def o4_head(paths):
    ...
    for retry in range(3):
        try:
            ...
            err_print(
                f"# {CLR}*** INFO: ({retry+1}/3) Retrieving HEAD changelist for", end
            )
            if not end:
                for path in args:
                    print(f"      {path}")
...
def parallel_fstat(opts):
    ...
    with NamedTemporaryFile(mode="w+t") as sin:
        ...
        return check_call(
            ["manifold", "-c", "4", "--", "xargs", "-n1", "o4", "fstat", "-q"],
            stdin=sin,
        )
...
def main():
    ...
    try:
        ...
        if opts["seed-from"]:
            ran = True
            o4_seed_from(
                opts["<dir>"], opts["--fstat"], "move" if opts["--move"] else "copy"
            )
    ...
    os.environ[
        "PWD"
    ] = os.getcwd()  # p4 ignores the actual directory and relies on $PWD
    ...
    if "@" in opts:
        if head_change < opts["@"]:
            err_print(
                f"*** INFO: Latest change on {os.getcwd()} is less than {opts['@']}.",
