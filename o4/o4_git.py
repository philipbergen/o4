...
def git_master_prep(depot_path, target_changelist, merge_target="master"):
    ...
    try:
        ...
        sys.exit(
            "Failed to git stash current changes, that probably means a conflict has to not"
...
def git_o4_import(prep_res):
    if prep_res["old_changelist"] >= prep_res["target_changelist"]:
        err_print(
            f"*** WARNING: o4 hybrid git sync is not possible for decreasing changelists"
            f" ({prep_res['old_changelist']} -> {prep_res['target_changelist']})"
        )
        return
    if not prep_res["old_changelist"]:
        err_print(
            "*** WARNING: There was no original import changelist, creating .p4_original_cl"
        )
        with open(".p4_original_cl", "wt") as fout:
            fout.write(f"{prep_res['target_changelist']}")
        err_check_call(["git", "add", ".p4_original_cl"])
    err_print(
        f"*** INFO: [git] Importing changes on {prep_res['depot_path']}"
        f"@{prep_res['old_changelist']},{prep_res['target_changelist']} "
        f"from perforce to git..."
    )
    res = check_output(
        ["git", "ls-files", "--others", "--exclude-standard"], universal_newlines=True
    )
    ...
    if untracked:
        ...
        with open(".o4/lfs.txt", "wt") as fout:
            for a in lfs:
                print(a, file=fout)
        err_print(
            f"*** INFO: [git] Adding {len(adding)} new files, and "
        ...
        if adding:
            err_print(
                f"*** INFO: [git] Adding {len(adding)} new files to git...", end=" "
            )
            while adding:
        ...
        if untracked:
            ...
            for u in untracked:
                err_print("   ", u)
            err_print(
                "*** WARNING: [git] The above listed files will be addeded to .gitignore"
            )
    ...
    err_print(
        f"*** INFO: [git] Committing changes from o4 to git {prep_res['merge_target']}..."
    )
    res = run(
        [
            "git",
    ...
    err_print(
        f"*** {'WARNING' if res.returncode else 'INFO'}: [git] Response:",
        res.stdout.replace("\n", "\n    "),
    )
...
def git_master_restore(prep_res):
    ...
    if git_current_branch() != prep_res["topic"]:
        err_print(
            f"*** INFO: [git] Returning git to topic branch {prep_res['topic']}..."
        )
        err_check_call(["git", "checkout", prep_res["topic"]])
        err_print(
            f"*** INFO: [git] Merging {prep_res['merge_target']} into "
    ...
    if ":" in prep_res["stash"]:
        ...
        try:
            ...
            err_print(
                "*** WARNING: [git] There was a problem popping the git stash, most likely "
                "a conflict.\n"
